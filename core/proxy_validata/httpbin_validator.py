...
检查代理IP速度和匿名程度：
    ...
    2.匿名程度检查：
        ...
        2.如果响应的origin 中有‘，’分割的两个IP就是透明的IP
        3.如果响应的headers 中包含proxy-Connection 说明是匿名代理IP
        4.否则就是高匿代理IP
检查代理IP协议类型
...
def __check_http_proxies(proxies, is_http=True):
    ...
    try:
        ...
        start = time.time()
        # 发送请求
        response = requests.get(test_url, headers=get_headers(), proxies=proxies, timeout=TEST_TIMEOUT)
        ...
        if response.ok:
            ...
            # 1.如果响应的origin中有‘，’分割的两个IP就是透明的IP
            if ',' in origin:
                nick_type = 2
            # 2.如果响应的headers中包含Proxy-connection说明是匿名代理IP
